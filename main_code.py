...
def batchDownloadImages(Paper_ID):
    ...
    image_dir = os.path.join(os.getcwd(), 'Batch_OMR_Image', Paper_ID)
    csv_dir = os.path.join(os.getcwd(), 'Batch_OMR_Output', Paper_ID)
    ...
    with ftplib.FTP(host=ftp_host, user=ftp_user, passwd=ftp_pass) as ftpServer:
        ftpServer.login()
        ftpServer.cwd(f'{ftp_file_path}/{Paper_ID}/OMR_Image')
        for ftp_file in ftpServer.nlst():
            print(ftp_file)
            ftpServer.retrbinary(f"RETR {ftp_file}", open(os.path.join(image_dir, ftp_file), 'wb').write)


def batchDeleteImageFiles(Paper_ID):
    try:
        image_dir = os.path.join(os.getcwd(), 'Batch_OMR_Image')
        for x in os.listdir(image_dir):
            if x == Paper_ID:
                shutil.rmtree(os.path.join(image_dir, Paper_ID))
        return (1, 'success')
...
def batchDeleteCSVFiles(Paper_ID):
    try:
        csv_dir = os.path.join(os.getcwd(), 'Batch_OMR_Output')
        for x in os.listdir(csv_dir):
            if x == Paper_ID:
                shutil.rmtree(os.path.join(csv_dir, Paper_ID))
        return (1, 'success')
    except Exception as e:
        error_msg = str(e)
        return (0, error_msg)
...
def batchProcessOMR(FTP_Path, FTP_Paper_ID, FTP_Batch_Name):
    try:
        ...
        for image_name in os.listdir(batch_images_dir):
            image_path = os.path.join(batch_images_dir, image_name)
...
def processOMR(FTP_Path, FTP_Paper_ID, FTP_Student_ID):
    try:
        emptyStoredFiles()
